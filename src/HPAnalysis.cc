...
void HPAnalysis::FillNtuple(std::vector<TrackInfo_t> trackInfoVec){
  ...
  Double_t massKaonPlus  = G4KaonPlus::KaonPlus()->GetPDGMass();
  Double_t massKaonMinus = G4KaonMinus::KaonMinus()->GetPDGMass();
  ...
  else if( pname == "kaon+"    ) massInc = massKaonPlus;
  else if( pname == "kaon-"    ) massInc = massKaonMinus;
  else exit (1);
